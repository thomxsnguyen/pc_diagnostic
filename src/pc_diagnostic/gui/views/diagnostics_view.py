...
def build_evidence_packet(
    ...
    cpu_model = "Unknown"
    model_readings = metrics.get("system.info.cpu_model", [])
    if model_readings:
        cpu_model = model_readings[0].tags.get("value", "Unknown")
    ...
    top_cpu_procs: list[dict[str, Any]] = [
        {
            ...
            "cpu": float(r.value),
            "mem": float(
                memory_by_pid.get((r.tags.get("pid", ""), "cpu_top"), 0.0)
            ),
        }
        for r in metrics.get("process.cpu_percent", [])
    ...
    top_mem_procs: list[dict[str, Any]] = [
        {
            ...
            "name": r.tags.get("name", "Unknown"),
            "cpu": float(
                cpu_by_pid.get((r.tags.get("pid", ""), "mem_top"), 0.0)
            ),
    ...
    return {
        ...
        "ram_used_str": _format_bytes(value(("memory.used",), 0.0)),
        "cpu_temp": value(
            ("system.temperature.cpu", "thermal.cpu_temp"), -1.0
        ),
        "gpu_temp": value(
            ("system.temperature.gpu", "thermal.gpu_temp"), -1.0
        ),
    ...
    class DiagnosticWorkerThread(QThread):
        ...
        def run(self) -> None:
            ...
            try:
                ...
                report = run_diagnosis(self.evidence)
...
class DiagnosticsView(QWidget):
    ...
    def _init_ui(self) -> None:
        ...
        self.recommendation_categories.setStyleSheet(
    ...
    def start_diagnosis(self) -> None:
        ...
        worker = DiagnosticWorkerThread(self._evidence, self)
        self._worker = worker
    ...
    def _on_diagnosis_finished(self, report: str) -> None:
        ...
        categories = self._recommendation_focus(self._evidence)
        self.recommendation_categories.setText(
            "Recommendation focus: " + " • ".join(categories)
        )
        for button in (
            self.save_markdown_button,
            self.save_html_button,
    ...
    @staticmethod
    def _recommendation_focus(evidence: dict[str, Any]) -> list[str]:
        categories: list[str] = []
        hottest = max(
            float(evidence.get("cpu_temp", -1.0)),
            float(evidence.get("gpu_temp", -1.0)),
        )
        if hottest > 65.0 or float(evidence.get("fan_speed", -1.0)) >= 0.0:
            categories.append("Hardware / cooling")
        if (
            float(evidence.get("cpu_util", 0.0)) > 55.0
            or float(evidence.get("ram_util", 0.0)) > 55.0
            or evidence.get("top_cpu_procs")
            or evidence.get("top_mem_procs")
            or evidence.get("active_incidents")
        ):
            categories.append("Software / processes")
        return categories or ["Preventive monitoring"]
    ...
    def _populate_evidence_tree(self, evidence: dict[str, Any]) -> None:
        ...
        groups = {
            ...
            "Thermals & Cooling": ("cpu_temp", "gpu_temp", "fan_speed"),
